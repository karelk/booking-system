...
function chooseDate(e) {
  ...
  var txtvalue = targ.getAttribute('date'); 
  txtvalue = prepareDate(txtvalue);
  var dt = new Date(txtvalue);
  textbox.value = formatDateYYYYmmdd(dt) // Set the selected date
...
function createCalendar(div, month) {
  ...
  dt = new Date(textbox.value);
  dt = formatDateddmmYYYY(dt);
  var selected = parseMyDate(dt); // Try parsing the date
  var today = new Date();
  ...
  while (1) { // Loop for each week
    ...
    for (i=0;i<7;i++) { // Loop each day of this week
	  ...
	  DisablePrevoiusDate(lastYearBn,inp,date);

    }
	DisableMonthYear(lastYearBn,lastMonthBn,date);
...
function DisableMonthYear(yrbtn,mnbtn,date) {

	var currtade = new Date();
	var daydate = date.getDate();
	var currdaydate = currtade.getDate();
	var monthdate = date.getMonth();
	var currmonthdate = currtade.getMonth();
	var monthdate = date.getMonth();
	var currmonthdate = currtade.getMonth();
	var yeardate = date.getFullYear();
	var curryeardate = currtade.getFullYear();
	...
	if(currmonthdate < 10)
	{
		currmonthdate = "0" + currmonthdate;
	}

	var Tdate = yeardate + "" + monthdate + "" +  daydate;
	var Cdate = curryeardate + "" +  currmonthdate + "" +  currdaydate;
	if( Tdate <= Cdate)
	{	
		mnbtn.disabled = "disabled";
		mnbtn.className ='disablemonth';		
	}

	curryeardate = (currtade.getFullYear() + 1);
	Cdate = curryeardate + "" +  currmonthdate + "" +  currdaydate;
	if( Tdate < Cdate)
	{
		yrbtn.disabled = "disabled";
		yrbtn.className ='disablemonth';
	}	
...
function DisablePrevoiusDate(yrbtn,dtbtn,date) {
	...
	if( Tdate <= Cdate)
	{	
		dtbtn.disabled = "disabled";
        dtbtn.className ='disablemonth';
	}
}

//fundtion to format date in yyyymmdd format
function prepareDate(date) {	
 
	var splChars = date.split('-')
	// CONCAT THE STRINGS IN YYYY-MM-DD FORMAT
	var datestring = splChars[2];
	
	if(splChars[1].length < 2)
	{
		datestring = datestring + '-0' + splChars[1];
	}else
	{
		datestring = datestring + '-' + splChars[1];
	}

	if(splChars[0].length < 2)
	{
		datestring = datestring + '-0' + splChars[0];
	}else
	{
		datestring = datestring + '-' + splChars[0];
	}
...
function formatDateddmmYYYY(date) 
{
	...
	// CONCAT THE STRINGS IN DD-MM-YYYY-DD FORMAT
